...
FIELDS = [
    {
        ...
        "validate": lambda v: (
            ...
            if not _PROJECT_ID_RE.match(v) else None
        ),
    },
    {
        ...
        "validate": lambda v: (
            ...
            if not _INSTANCE_NAME_RE.match(v) else None
        ),
    },
    {
...
def _write_tfvars(path: str, config: dict) -> None:
    ...
    nonprod_project = config.get("nonprod_project_id", "")
    image = f"gcr.io/{nonprod_project}/{config.get('job_name', 'cloudsql-sync')}:latest"
    alert_email = config.get("alert_email", "")

    with open(path, "w") as f:
        f.write("# CloudSQL Sync — Terraform variables\n")
        f.write("# Generated by configure.py — do not commit this file\n\n")
        f.write(f'prod_project_id       = "{config.get("prod_project_id", "")}"\n')
        f.write(f'prod_instance_name    = "{config.get("prod_instance_name", "")}"\n\n')
        f.write(f'nonprod_project_id    = "{nonprod_project}"\n')
        f.write(f'nonprod_instance_name = "{config.get("nonprod_instance_name", "")}"\n\n')
        f.write(f'region     = "{config.get("region", "us-central1")}"\n')
        f.write(f'job_name   = "{config.get("job_name", "cloudsql-sync")}"\n')
        f.write(f'schedule   = "{config.get("schedule", "0 2 * * *")}"\n')
        f.write(f'timezone   = "{config.get("timezone", "UTC")}"\n\n')
        f.write(f'# Build image first: gcloud builds submit sync_job/ --tag={image}\n')
        f.write(f'container_image = "{image}"\n\n')
        if alert_email:
            f.write(f'alert_email = "{alert_email}"\n')
        else:
            f.write('# alert_email = "your-team@example.com"\n')
