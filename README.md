...
**Requirements:** the Secret Manager password reset must be enabled (the job connects as `postgres` to run the grants), and Terraform grants each target identity `roles/cloudsql.instanceUser` on the hosting project so IAM login actually succeeds. Role and identity names are validated against a strict charset before ever reaching SQL.
