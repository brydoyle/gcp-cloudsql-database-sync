...
| 5 | `instances.get` + `SELECT 1` (per target) | Post-restore verification: state must be `RUNNABLE`; live SQL check when the password reset is enabled (`verify_restore`, default on) |
| 6 | `backupRuns.delete` (prod) | Delete the backup — **only if this job created it**; a reused backup is left in place |

Step 6 runs in a `finally` block — it always executes even if a restore fails, and its own failure is non-fatal (warns only) so a cleanup error never masks a successful sync. Ownership is explicit: `acquire_backup()` returns `(backup_id, created_by_us)` and cleanup only fires when `created_by_us` is true.
...
| Service Account `cloudsql-sync@nonprod` | nonprod | Job identity — narrow custom roles on prod + each target (Terraform default), or `cloudsql.admin` (deploy.sh / `least_privilege=false`) |
| Custom roles `<job>_backup_ops` / `<job>_restore_ops` | prod / targets | Least-privilege permission sets for the job SA (Terraform only) |
...
| Alert policy `cloudsql-sync sync overdue` | nonprod | Fires if no success in 23.5h — **daily-or-finer schedules only** (Cloud Monitoring alerting can't look back >24h, so weekly/monthly cadences rely on the failure alert); omitted when on-demand |
...
### 1. Networking — Public IP ⚠️ PARTIALLY ADDRESSED

**Job egress is now configurable** (README "Networking"): public by default so the tool works in any environment, with opt-in **VPC connector** or **Direct VPC egress** via `vpc_connector` / `vpc_network` (+ `vpc_egress`). Public mode is loud by design — it warns at deploy time (`deploy.sh`), plan time (Terraform `check "public_networking"`), and **at runtime in the job's logs** (`SYNC_NETWORK_MODE`).

**What remains for production:** the Cloud SQL **instances** themselves must be given Private IPs when provisioned (see the commented block in `terraform/examples/target-instance.tf.example`), and the chosen VPC path must route to them (at corporate scale, via the existing Dedicated Interconnect).
...
- Service accounts follow least-privilege: with Terraform's default `least_privilege = true`, the job SA holds narrow custom roles (backup ops on prod, restore/connect/user-update on targets) — it cannot delete or reconfigure any instance
